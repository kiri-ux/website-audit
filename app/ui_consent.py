...
def _capture_panel(aid, url, why, heading="This scan ran without a browser",
    ...
    return (
        ...
        f"unzip it somewhere <b>permanent</b> &mdash; a folder you keep, not "
        f"Downloads &mdash; and load it unpacked with Developer mode on. "
        f"Chrome remembers the FOLDER, not the extension: move or clear that "
        f"folder and the extension disappears on the next restart, which is "
        f"the usual reason it goes missing. Audit id <code>{e(aid)}</code> "
        f"{_copy(aid)}</p></div>"
